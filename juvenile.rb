...
config['apps'].each do |app, settings|
  ...
  Helpers.output log, "info",  "- Uploading #{dump_name} to S3"
  
  upload_progress = 0
  file = File.open(File.join(File.dirname(__FILE__), dump_name), 'rb', encoding: 'BINARY')
  opts = {
    estimated_content_length: file.size,
  }
  part_size = Helpers.compute_part_size(opts)
  
  new_obj = bucket.objects["#{s3_subdirectory}/#{dump_name}"]
  begin
  ...
  Helpers.output log, "info",  "- Finished uploading #{dump_name} to S3"
  
  #Confirm the upload
  Helpers.output log, "info",  "- Confirming upload"
  confirmed = bucket.objects["#{s3_subdirectory}/#{dump_name}"]
  if confirmed.exists?
    ...
    Helpers.output log, "error", "- #{dump_name} not found"
    results << "Error backing up #{app}"
  end

  #Delete the mysqldump file locally
  Helpers.output log, "info",  "- Deleting local copy of #{dump_name}"
